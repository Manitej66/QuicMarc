...
const Details = () => {
  ...
  const [value, loading, error] = useDocumentData(
    firestore.collection("attendance").doc(code),
    {
  ...
  const renderer = ({ hours, minutes, seconds, completed }) => {
    if (completed) {
      return (
        <p className="p-2 bg-red-800 text-red-100 text-center shadow-md rounded-md">
          Timeout
        </p>
      );
    } else {
  ...
  return (
    <div className="flex flex-col justify-center items-center font-myfont p-4">
      <div>
        <p className="text-blue-600 text-4xl font-extrabold text-center">
          {value.name}
        </p>
        <Countdown date={value.endsAt} renderer={renderer} />
